...
modeGridJS.buildPageHTML = () => {
    ...
    let sortedPosts = modeGridJS.sortByDate(entryDB);
    ...
    function postsDiv(sortedPosts) {
        let result = ``;
        result += `<div class="main--grid">`;
        sortedPosts.forEach((post, index) => {
            let classList = `main--grid--item-${index} main--grid--item`;
            let innerResult = ``;
    ...
    function postsDiv(sortedPosts) {
        ...
        sortedPosts.forEach((post, index) => {
            ...
            innerResult += `<div class="${classList}--lower">`;
            innerResult += `<p class="${classList}--lower--body">${text.ellipsize(post.body)}</p>`;
            innerResult += `<div class="${classList}--lower--buttons">
                                <form action="/edit/${post.date.date.code}" method="GET" class="${classList}--lower--buttons--edit form-group">
                                    <button class="btn btn-primary" name="submit" type="submit">Edit</button>
                                </form>
                                <form action="/delete/${post.date.date.code}" method="POST" class="${classList}--lower--buttons--delete form-group">
                                    <button class="btn btn-primary" name="draft" type="submit">Delete</button>
                                </form>
                            </div>`;
            innerResult += `</div>`;
